...
public class FireScriptRunner {
  ...
  public FireScriptRunner(Connection connection) {
  ...
  /**
   * 自定义方法,目的在同一事务中执行多个sql文件，单个runScriptTransaction()不提交事务，需要手动提交
   * @param reader
   */
  public void runScriptTransaction(Reader reader) {
    //setAutoCommit();
    try {
      if (sendFullScript) {
        executeFullScript(reader);
